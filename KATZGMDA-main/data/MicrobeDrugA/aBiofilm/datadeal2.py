...
# 将包含逗号的字符串列表转换为浮点数列表，并保留小数点后10位
float_lines = [[format(float(value), '.10f') for value in line] for line in lines]

# 将转换后的浮点数列表写入新的文本文件，保持原始的行列结构
with open('drug_mic_matrix8.txt', 'w') as file:
    for line in float_lines:
        file.write(','.join(line) + '\n')
a=np.loadtxt("C:\新建文件夹\MKGCN-main\data\MicrobeDrugA\\aBiofilm\drug_mic_matrix8.txt",delimiter=',')
print(a.shape)
print(a)